		      ...
		      if (s instanceof Player) {
		        ...
		        if (m.hasPermission("spartacube.repair")) {
		          ...
		          if (delaymap.containsKey(m.getName())) {
		            ...
		            if (differenceInMin < CoreBukkitPlugin.getInstance().getDelay()) {
		              int time = (int)(CoreBukkitPlugin.getInstance().getDelay() - differenceInMin);
		...
		private void RepairCheck(String[] args, Player m) {
		    ...
		    if (args.length == 0 || (args.length >= 1 && args[0].equalsIgnoreCase("hand"))) {
		        ...
		        if (m.getInventory().getItemInHand().getItemMeta().hasLore()) {
		          if (CoreBukkitPlugin.blockall) {
		            ...
		            for (int i = 0; i < CoreBukkitPlugin.getInstance().getLores().length; i++) {
		              if (lore.contains(CoreBukkitPlugin.getInstance().getLores()[i])) {
		                m.sendMessage(ChatColor.RED + CoreBukkitPlugin.getInstance().getMSG());
		      ...
		      for (ItemStack item : m.getInventory()) {
		        if (item != null) {
		          Material material = item.getType();
		          if (item.hasItemMeta() && !CoreBukkitPlugin.blockall) {
